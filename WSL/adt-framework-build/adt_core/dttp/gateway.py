...
class DTTPGateway:
    ...
    def request(self,
        ...
        if normalized_path in CONSTITUTIONAL_PATHS:
            ...
            # Require explicit file match in spec (no directory wildcards for Tier 2)
            ...
            if not explicit_match or not tier2_justification:
                ...
                self.logger.log(ADSEventSchema.create_event(
                    ...
                    description=f"DENIED: Tier 2 path {normalized_path} requires explicit spec listing and justification. Rationale: {rationale}",
                    spec_ref=spec_id,
                    authorized=False,
                    tier=2,
                    escalation=True
